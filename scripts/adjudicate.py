...
【关键注意】
1. 以声明的完整专名为匹配基准，区分主题修饰词。
   - "防性侵\"六个一\""与"近视防控\"六个一\""是两个不同专项，"六个一"是通用词，
     "防性侵"/"近视防控"才是区分性修饰词。
   - 在证据中找到与声明完全同名的那一条后，直接判 confirmed；
     若证据仅含其他主题的"六个一"（或类似通用词），才判 not_found。
2. 直接命中规则：若证据片段中出现与声明同名的专项标题（如声明"防性侵'六个一'"，
   证据含"4. 防性侵'六个一'：……"），即为直接命中，判 confirmed，不需要核对列举细节。
3. 证据片段可能混杂多个不相关条目（如同时列出防性侵、近视防控、溺水防范等），
   需聚焦与声明主题一致的那一条，不受其他条目内容干扰。
4. 不得用常识判断；给定证据片段中无任何支持时选 not_found。
